...
const addModalToggleControl = ( BlockEdit ) => {
	return ( props ) => {
		...
		const { openInModal = false, modalContentSelector = ".entry-content" } = attributes;
		...
		return (
			<Fragment>
              ...
              __next40pxDefaultSize
								...
								help={ __( "Enter a CSS class selector to identify the content to pull in. Defaults to .entry-content.", "mello-block-extensions" ) }
								value={ modalContentSelector }
								onChange={ ( value ) => setAttributes( { modalContentSelector: value } ) }
							/>
						) }
					</InspectorControls>
				) }
