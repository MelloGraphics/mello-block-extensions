...
const buildExtensionEntries = () => {
  ...
  // Collect JavaScript entries (e.g., edit.js, frontend.js)
  const jsFiles = [
    ...glob.sync(path.resolve(__dirname, "../src/extensions/**/edit.js")),
    ...glob.sync(path.resolve(__dirname, "../src/extensions/**/frontend.js")),
  ];
  ...
  const styleFiles = glob.sync(
    path.resolve(__dirname, "../src/extensions/**/+(styles.scss|editor.scss)")
  );
  styleFiles.forEach((file) => {
    ...
    if (parsed.name === "editor") {
        outputName = "editor-style";
      }
    const entryName = path.join(path.dirname(relativePath), outputName);
    entries[entryName] = file;
  });

  console.log("Entries for extensions build:", entries);
